...
#1.檢定A班與B班的平均身高是否有差異(大樣本, 母體標準差A班=4, B班=3)
...
#3.檢定A班平均體重是否大於B班3公斤(大樣本, 母體標準差A班=3, B班=3)
...
#1.平均的花萼和花瓣長度是否有差異?
z.test(iris$Sepal.Length,iris$Petal.Length,
       "two.sided", mu=0,
       sigma.x=sqrt(var(iris$Sepal.Length)),
       sigma.y=sqrt(var(iris$Petal.Length)),
       conf.level=0.95)

#2.平均的花萼寬度是否小於花瓣寬度1公分?
z.test(iris$Sepal.Width,
       iris$Petal.Width,
       "less",
       mu=1,
       sigma.x=sqrt(var(iris$Sepal.Width)),
       sigma.y=sqrt(var(iris$Petal.Width)),
       conf.level=0.95)
...
matrix <- matrix(1:100, nrow=5, ncol=20,byrow = TRUE)
matrix
apply(matrix,1,mean)

#2.建一個list -> 第一維放一個matrix(1~4, 2列2行, 依照column填入資料)
#第二維放vector(5~20), 得出這兩維的最小值
list <-list(matrix(1:4, nrow=2, ncol=2),c(5:20))
list
sapply(list, min)
...
#每周不同學習時間的G2成績是否相等?
#step1:常態性檢定
shapiro.test(log(dataset2$G2))#<0.05, reject
shapiro.test(sqrt(dataset2$G2))#<0.05, reject
#假設他成常態


#step2:變異數同質檢定
leveneTest(G2 ~ as.factor(studytime), data = dataset2)#<0.05, reject
#假設他母體資料變異數相等
...
summary(res3)
TukeyHSD(res3, conf.level=.95)
plot(TukeyHSD(res3, conf.level=.95), las = 2)
